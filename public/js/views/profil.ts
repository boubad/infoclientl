...
import {FileDesc} from '../data/filedesc';
import {IDatabaseManager, IPerson, IElementDesc, IFileDesc} from 'infodata';
import {EMPTY_STRING} from '../data/infoconstants';
//
//
@autoinject
export class Profil extends InfoBaseView {
  //
  ...
  public changePwd(): any {
    ...
    this.clear_error();
    return this.dataService.dm_maintains_item(pPers).then((r) => {
      self.infoMessage = 'Mot de passe modifi̩';
  ...
  public saveData(): any {
    ...
    this.clear_error();
    return this.dataService.dm_maintains_item(pPers).then((r) => {
      self.infoMessage = 'Informations enregistr̩es!';
    }, (err) => {
        self.set_error(err);
      });
  }// saveData

...
import {GROUPE_TYPE, GROUPE_PREFIX, PROFAFFECTATION_BY_GROUPE, ETUDAFFECTATION_BY_GROUPE} from '../infoconstants';
//
export class Groupe extends DepSigleNameItem implements IGroupe {
    ...
    public remove(service: IDatabaseManager): Promise<any> {
        ...
        let docids: string[] = [];
        return service.get_children_ids(PROFAFFECTATION_BY_GROUPE, id).then((aa_ids) => {
            if ((aa_ids !== undefined) && (aa_ids !== null)) {
                for (let x of aa_ids) {
                    self.add_id_to_array(docids, x);
                }
            }
            return service.get_children_ids(ETUDAFFECTATION_BY_GROUPE, id);
        }).then((uu_ids) => {

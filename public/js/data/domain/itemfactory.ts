...
export class ItemFactory extends InfoElement implements IItemFactory {
    ...
    public create_person(oMap: any): IPerson {
        ...
        if (type === ETUDIANTPERSON_TYPE) {
            vRet = new EtudiantPerson(oMap);
        } else if (type == ENSEIGNANTPERSON_TYPE) {
            vRet = new EnseignantPerson(oMap);
        } else if (type == ADMINISTRATORPERSON_TYPE) {
            vRet = new AdministratorPerson(oMap);
        } else if (type == PERSON_TYPE) {
            vRet = new Person(oMap);
        }
        return vRet;
    }
}// class ItemFactory

...
import {IBaseItem, IItemFactory, IEtudAffectation, IEtudEvent,
IProfAffectation, IGroupeEvent,
IDepartement, IAnnee, IUnite, IMatiere, ISemestre, IGroupe,
IPerson, IDatabaseManager} from 'infodata';
import { NOTES_BY_SEMESTRE_MATIERE, ETUDEVENTS_BY_SEMESTRE_EVTS,
	ETUDEVENTS_BY_PERSON,GROUPEEVENTS_BY_SEMESTRE_MATIERE_GROUPE } from '../../utils/infoconstants';
//
export class DatabaseManager extends DesignDatabase implements IDatabaseManager {
  ...
  //
  public dm_get_etudiant_events(personid: string): Promise<IBaseItem[]> {
	...
	//
	public dm_get_children_ids(viewName: string, keyVal: string): Promise<string[]> {
		return this.get_children_ids(viewName,keyVal);
	}
	public dm_maintains_doc(doc: any): Promise<any> {
		let xdoc = <IPouchDocument>doc;
		return this.maintains_doc(xdoc);
	}
    //
    ...
    public dm_get_semestre_matiere_notes(semestreid: string, matiereid: string): Promise<IBaseItem[]> {
        let oRet: IBaseItem[] = [];
        if ((semestreid === undefined) || (semestreid === null) ||
    ...
    public dm_get_semestre_evts_ids(semestreid: string): Promise<string[]> {
        if ((semestreid === undefined) || (semestreid === null)) {
            return Promise.resolve([]);
        }
        let self = this;
	...
	public dm_get_semestre_matiere_groupe_grpevts_ids(semestreid: string,
		matiereid:string, groupeid:string): Promise<string[]> {
        if ((semestreid === undefined) || (semestreid === null) ||
			(matiereid === undefined) || (matiereid === null) ||
			(groupeid === undefined) || (groupeid === null)) {
            return Promise.resolve([]);
        ...
        let self = this;
        let startKey = [semestreid,matiereid,groupeid];
        let endKey = [semestreid, matiereid,groupeid];
        let options: PouchQueryOptions = {
            startkey: startKey, endkey: endKey
        };
        let viewname = GROUPEEVENTS_BY_SEMESTRE_MATIERE_GROUPE ;
        let oRet: string[] = [];
        return this.db.then((xdb) => {
            return xdb.query(viewname, options);
        }).then((result) => {
    ...
    public dm_get_semestre_evts(semestreid: string): Promise<IBaseItem[]> {
        let oRet: IBaseItem[] = [];
        if ((semestreid === undefined) || (semestreid === null)) {
            return Promise.resolve(oRet);
        }

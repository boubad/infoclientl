...
export class PersonViewModel<T extends IDepartementPerson, V extends IPerson>
    ...
    protected import_persons(dd: V[]): Promise<boolean> {
        ...
        let self = this;
        return service.dm_check_items(pp).then((zz: IPerson[]) => {
            let ppp: T[] = [];
            for (let z of zz) {
            ...
            if (ppp.length > 0) {
                return service.dm_check_items(ppp);
            } else {
                return [];
            }
        }).then((w) => {
    ...
    public saveAvatar(): any {
        ...
        return service.maintains_attachment(id, avatarid, data, type).then((r) => {
            ...
            pp.url = p.url;
            self.fileDesc.clear();
            self.infoMessage = 'Avatar modifiÃƒÂ©.';
        }).catch((err) => {
            self.set_error(err);
        });
    }// saveAvatar
    ...
    public removeAvatar(): any {
        ...
        if (this.confirm('Voulez-vous vraiment supprimer cet avatar?')) {
            ...
            return service.remove_attachment(id, avatarid).then((r) => {
                ...
                self.fileDesc.clear();
                self.infoMessage = 'Avatar supprimÃƒÂ©.';
            }).catch((err) => {
                self.set_error(err);
            });
        }
    ...
    public save(): any {
        ...
        this.clear_error();
        let service = this.dataService;
        return service.dm_maintains_item(pPers).then((px) => {
            return service.dm_maintains_item(item);
        }).then((ox) => {
            if (bOld) {
                self.refresh();
            } else {
